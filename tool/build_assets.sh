...
mkdir -p "$RIG/full"
# The crop box applies to the THUMBNAIL only. Tiles are a uniform 4:3 contact
# sheet, but the viewer opens the whole frame — cropping both would mean the
# photo could never be seen in full anywhere on the site.
rig() {   # slug x y w h   (x y w h crop the 1000px tile; full/ is uncropped)
